...
std::vector<Cube::Rotation> find_solve_through_ida_star(
    ...
    std::priority_queue<PriorityNode, std::vector<PriorityNode>, std::greater<>> tmp_nodes;
    std::array<Cube::Rotation, MaxRotationsSize> rotations;

    uint8_t current_optimal_estimate = heuristic.GetEstimatedMovesNumber(cube);
    uint8_t max_depth = 0;
    ...
    while (true) {
        ...
        while (!tmp_nodes.empty()) {
            nodes.emplace(tmp_nodes.top().cube, current_node.depth + 1, tmp_nodes.top().rotation);
            tmp_nodes.pop();
        }
